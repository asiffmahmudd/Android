...
public class Logic 
    ...
    */
    ...
    public static int simulation(long seed, int size){
        Random random = new Random();
        random.setSeed(seed);
        int birthday[] = new int[365];
        int f = 0, r = 0;
        for(int i = 0; i < size; i++) {
            r = random.nextInt(365);
            if (birthday[r] > 0){
                f = 1;
                break;
            }
    ...
    public double calculate(int size, int count) {
        ...
        for(int i = 1; i <= count; i++){
            simulation_result += simulation(i,size);
        }
        return (Double.valueOf(simulation_result*100.00)/count);
    }
    // TODO - add your code here
}
